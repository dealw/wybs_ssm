...
<body class="hold-transition skin-purple sidebar-mini">
	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<h1>
				旅客管理 <small>添加旅客</small>
			</h1>
			<ol class="breadcrumb">
				<li><a href="${pageContext.request.contextPath}/index.jsp"><i
						class="fa fa-dashboard"></i> 首页</a></li>
				<li><a
					href="${pageContext.request.contextPath}/user/findAll.do">数据管理</a></li>
					<a href="${pageContext.request.contextPath}/user/findAll.do">数据列表</a></li>
				<li class="active">用户表单</li>
			</ol>
			</section>
			<!-- 内容头部 /-->

			<form action="${pageContext.request.contextPath}/orders/addTravellerToOrder.do"
				method="post">
			...
			<form
					action="${pageContext.request.contextPath}/orders/deleteTravellerInOrder.do"
				...
				<section class="content">
					...
					<table id="dataList"
						...
						<tbody>
						<c:forEach items="${travellers}" var="travller">
							<tr>
								<td>
									<input name="ids" type="checkbox" value="${travller.id}" id="ids">
								</td>
								<td>${travller.id}</td>
								<td>${travller.travellerTypeStr }</td>
								<td>${travller.name}</td>
								<td>${travller.sex}</td>
								<td>${travller.phoneNum}</td>
								<td>${travller.credentialsTypeStr}</td>
								<td>${travller.credentialsNum}</td>
							</tr>
						</c:forEach>
						</tbody>
					</table>
					<!--订单信息/--> <!--工具栏-->
					<div class="box-tools text-center">
						<button type="button" class="btn bg-maroon" onclick="del()">移除</button>
						<button type="button" class="btn bg-default"
								onclick="history.back(-1);">返回</button>
					</div>
					<!--工具栏/--> </section>
